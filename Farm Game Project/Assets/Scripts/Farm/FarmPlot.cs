...
public class FarmPlot : MonoBehaviour
{
    ...
    [SerializeField] public FarmCrops _farmCrops;
    ...
    void Start()
    {
        _farmCrops.crops = _farmCrops.rice;
    ...
    private void CropGrowth()
    {
        switch (_growthState)
        {
            ...
            case State.Seeded:
                if (!_hasPlantedSeeds)
                {
                    _plantedSeeds = Instantiate(_farmCrops.crops.GetSeed, transform);
            ...
            case State.FullGrown:
                if (!_isHarvestCrop)
                {
                    _harvestCrop = Instantiate(_farmCrops.crops.GetCrop, transform);
                    _isHarvestCrop = true;
                }
                break;
        }
    ...
    {
        _growthTime = _farmCrops.crops.GetGrowthTime;
